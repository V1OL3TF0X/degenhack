...
const env = { supportedChains: [alephzeroTestnet.network] };
      ...
      {supportedChains.map((chain) => (
        ...
        >
          <div className='flex w-full items-center justify-between gap-2'>
            <p>{chain.name}</p>
            {chain.network === activeChain?.network && <TaskAltIcon className='shrink-0' />}
          </div>
        </MenuItem>
      ))}
      {(accounts || []).map((acc) => {
        ...
        return (
          ...
          >
            <div className='flex w-full items-center justify-between'>
              ...
              {acc.address === activeAccount?.address && <TaskAltIcon className='shrink-0' />}
            </div>
          </MenuItem>
        );
      })}
...
export const AccountName: FC<AccountNameProps> = ({ account, ...rest }) => {
  ...
  const { primaryDomain } = useResolveAddressToDomain(doResolveAddress ? account?.address : undefined, {
